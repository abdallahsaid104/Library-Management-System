class LoanRepository:
    ...
    def get_active_loan(self, barcode):
        return self.db.fetch_query("SELECT loan_id, due_date FROM loans WHERE book_barcode = ? AND return_date IS NULL",
                                   (barcode,)
                                   )

    def get_active_loan_for_isbn(self, isbn):
        return self.db.fetch_query("""SELECT l.loan_id FROM loans l JOIN book_items bi ON l.book_barcode = bi.barcode
    ...
    def close_loan(self, loan_id, return_date, fine):
        self.db.execute_query("UPDATE loans SET return_date = ?, fine_amount = ? WHERE loan_id = ?",
                              (str(return_date), fine, loan_id)
                              )

    def get_borrower_info(self, barcode):
        return self.db.fetch_query("""SELECT m.name, m.member_id, m.email, l.due_date FROM loans l 
        JOIN members m ON l.member_id = m.member_id WHERE l.book_barcode = ? AND l.return_date IS NULL""", (barcode,))

    def get_borrowed_books_by_member(self, member_id):
        return self.db.fetch_query("""SELECT b.title, l.book_barcode, l.due_date  FROM loans l 
    ...
    def get_all_active_loans(self):
        return self.db.fetch_query("""SELECT m.name, m.member_id, b.title, l.book_barcode, l.due_date FROM loans l
    ...
    def get_overdue_loans(self, today):
        return self.db.fetch_query("""SELECT l.member_id, l.book_barcode, l.due_date, b.title FROM loans l
                    JOIN book_items bi ON l.book_barcode = bi.barcode JOIN books b ON bi.isbn = b.isbn
